# fLoc
Functional localizer experiment used to define category-selective cortical regions (see [Stigliani et al. (2015)](http://www.jneurosci.org/content/35/36/12412))
...
Stimulus sequences are generated by an object of the class [`fLocSequence`](https://github.com/VPNL/fLoc/blob/master/functions/fLocSequence.m). See instructions below for modifying the various parameters of the localizer design. 

#### Using specific image categories

To customize which image subcategory is used for each stimulus condition (without changing the overall conditions), edit the `fLocSequence` class file as detailed below: 

1. Add a constant property to `fLocSequence` called `stim_set3` that lists a one subcategory for each element in `fLocSequence.stim_conds`. For example:
```
...
We strongly recommend including each of the five main stimulus conditions (bodies, characters, faces, objects, places) regardless of the specific region/s you are interested in defining, as this provides a more stringent test of selectivity compared to only contrasting responses to a few other categories. To include additional conditions in the localizer, follow the instructions below: 

1. Generate a set of at least 144 images for stimulus condition of your choice, and place them in a separate directory in `~/fLoc/stimuli/` named after the specific subcategory used to represent that stimulus condition. 
2. Append the name of the custom stimulus condition to the cell array of conditions labels stored in the constant `fLocSequence`  property `stim_conds`. 
3. Append the name of the new image category in `~/fLoc/stimuli` to the list of subcategories in the constant `fLocSequence` properties `stim_set1` and `stim_set2`. 
4. Note that changing the number of stimulus conditions will affect dependent properties of `fLocSequence` such as run duration (`run_dur`).

#### Customizing presentation parameters

Other aspects of the experimental design can be modified by changing various properties in the `fLocSequence` class file.

- *Stimulus duty cycle* — the duration of each stimulus cycle can be modified by editing the constant property `fLocSequence.stim_duty_cycle`.
- *Stimuli per block* — the number of stimuli in a block is set in the constant property `fLocSequence.stim_per_block`. This will affect dependent properties such as run duration (`run_dur`).
